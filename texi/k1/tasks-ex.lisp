...
;; this is a tasks which finish normal
...
#+:SYMBOLICS(DEFBEHAVIOR (decide-what-to-do :get-all-inactive-tasks) (&rest arg)
              (declare (ignore arg))
              (if (<- self :is-active)
                `(:ONE-OF . ,(loop for a-task in (send-kb :instances)
                                   when (not (<- a-task :is-active))
                                   collect a-task))
                :ANY))
...
;;;
;;;      ($send (get-task 'my-task-1) :start)

;(CREATE-TASK my-task-1 mini-frplx-task tasks&agendas-examples 
;	     ((<- the-controller :inform)
;	      (<- the-controller :start)
;	      ($send self :print-hypotheses-verified)
;	      ($send self :print-true-facts))
;
;
;(PRESET-INFERENCE-TASK (get-task 'my-task-1))
;
