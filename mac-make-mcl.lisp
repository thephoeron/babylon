...
;;  FILE:    babylon^mac-make-MCL.lisp

;;           This is the make file for making a Babylon Image
;;           on an Apple Macintosh using Apple Macintosh Common Lisp 2.0

;;           use file babylon^mac-make-CCL.lisp 
;;           if you have Macintosh Allegro Common Lisp

;;           remember to edit file babylon-init.lisp to set the ccl logical host

(unless (member :MCL *features*)
  (progn (message-dialog "Sorry! This is the load file for Macintosh Common Lisp 2.0")
         (quit)))
...
(defvar *mcl-directory* 
  (concatenate 'string (namestring (translate-logical-pathname "ccl:")) "**:*.*"))

;;; for a CLtL II implementation make package "USER" 
;;; a nickname of package "COMMON-LISP-USER"
...
(if (y-or-n-p "Use development options for compiling files?")
  (development-options)
  (runtime-options))
...
(defun clear-babylon ()
  (dolist (file (append (directory "Babylon:mac-binary;**;*.xfasl")
                        (directory "Babylon:tty;**;*.xfasl")
                        (directory "Babylon:samples;**;*.xfasl"))
                'done)
    (delete-file file)))
...
(defun babylon-initialize-image ()
  ...
  (setf (logical-pathname-translations "ccl")
        `(("ccl:inspector;**;*.*" "ccl:library;inspector folder;**;*.*")
          ("ccl:interfaces;**;*.*" "ccl:library;interfaces;**;*.*")
          ("ccl:**;*.*" ,*mcl-directory*)))
  (SET-CHOOSE-FILE-DEFAULT-DIRECTORY "babylon:")
  (babylon-hallo)
  (if (or (probe-file "babylon:patches.lisp")      ; loading patches
...
;(load "babylon:mac;compiler-warnings.lisp")
    ...
    (if (not (eql :cancel file))
      (return-from-modal-dialog
       (progn
         ...
         (eval-enqueue `(progn 
                          ;;; preload dynamically loaded parts
...
;                                :toplevel-function #'ccl::compiler-warning-muffler)))))))
;                                :compress
;                                ,(check-box-checked-p (view-named 'compress dialog))))))))))
; do not know syntax of :memory-options arg to save-application yet
        ...
        (make-instance 'dialog
          ...
          (list
           ...
           (make-instance 'check-box-dialog-item
             ...
             :view-position #@(122 250)
             :view-size #@(102 19)
             :check-box-checked-p nil
             :view-nick-name 'compress)
...
;;; to shut up compiler-warnings s. file "babylon:mac;compiler-warnings.lisp"
