...
(def$method (basic-rule-processor :print) (&optional (stream *standard-output*))
  "Unparse rules and rulesets on <stream>."
  ($send self :unparse-rules stream))

(def$method (basic-rule-processor :kb-inform) (&optional (stream *standard-output*))
  "Print statistics about rules and rulesets on <stream>."
  ($send self :rule-statistics stream))


#+:flavors(compile-$flavor-$methods basic-rule-processor)
