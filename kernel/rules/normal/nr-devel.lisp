...
(defun make-rule-header (rule-name item-len)
  ...
  (let ((str (the simple-string 
                  (format nil (getentry rule-fstr rule-io-table) rule-name))))
...
(defun make-term-item (term mode rules item-len spaces-for-item)
  ...
  (let ((str (the simple-string 
                  (format nil "~A~A" spaces-for-item (from-list-to-string term)))))
    `(,(COMPLETE-TO-N str (- item-len (length str)))
      ,@(if (null rules)
          `(:no-select t)
