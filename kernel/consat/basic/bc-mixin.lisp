...
(def$method (basic-constraint-mixin :after :new&delete) (&rest dummy)
...
(def$method (basic-constraint-mixin :eval-satisfy) (expression &rest dummy)
...
(def$method (basic-constraint-mixin :eval-satisfied-p) (expression &rest dummy)
...
(defun display-constraint (&rest dummy)
  (declare (ignore dummy))
  (catch 'no-select
    (send-kb :display)))
...
(defun read-constraint (&rest dummy)
  (declare (ignore dummy))
  (catch 'no-select
    (send-kb :read)))
