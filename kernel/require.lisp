...
#+(or :CMU)                     ;;; old module handling functions
(progn
  (defvar *modules* nil)
  ...
  (defun require (module &optional pathname)
    (let ((m (if (symbolp module) (symbol-name module) module)))
      (unless (member m *modules* :test #'string-equal)
        (load (or (probe-file m) pathname)))))
  )
...
(defun bab-require (modul-name &optional pathlist)
  (unless (find (string modul-name) *modules* :test #'string-equal)
